...
describe('FIELD_RESOLVERS map (schema audit)', () => {
  ...
  test('caseId → case_id, motionType → document_type (alias)', () => {
    expect(FIELD_RESOLVERS.caseId.column).toBe('case_id');
    expect(FIELD_RESOLVERS.motionType.column).toBe('document_type');
  });
  test('every resolver is lance-scalar (no sqlite round-trips)', () => {
    for (const r of Object.values(FIELD_RESOLVERS)) {
      expect(r.kind).toBe('lance-scalar');
    }
  });
});
...
describe('extractFieldFilters — unknown fields fall back', () => {
  ...
  test('purple:cow → bare text match against literal "purple:cow"', () => {
