...
/**
 * Reserved native MCP path for the `local` profile (REPORT-v2.1 Part E.1).
 * 404s until the Streamable HTTP transport lands; today clients register the
 * stdio bridge with SOUND_SUITE_PROFILE=local instead.
 */
  ...
  return NextResponse.json(
    {
      error: {
        code: 'NOT_AVAILABLE',
        message: 'Native MCP transport not yet available; register the stdio bridge with SOUND_SUITE_PROFILE=local',
      },
    },
    { status: 404 },
  );
