...
const CODE_EMBEDDING_MODELS: Array<{
  ...
  { name: 'qwen3-embedding:4b', label: 'Qwen3 Embedding 4B — Q4_K_M (4B params, 2560 dims, 32K context)', size: 2500 * 1024 * 1024, dims: 2560, about: 'General-purpose Qwen3 embedding, 4B parameters, 2560 dims. 4-bit weights: fastest and smallest, but its vectors do NOT match a full-precision server of the same model (measured cosine 0.976 against OpenRouter/DeepInfra), so the two cannot share one index.' },
  { name: 'qwen3-embedding:4b-q8_0', label: 'Qwen3 Embedding 4B — Q8_0 (2560 dims, closer to full precision)', size: 4300 * 1024 * 1024, dims: 2560, about: '8-bit weights. Middle ground: nearer a full-precision server than Q4_K_M, still smaller and faster than fp16. Verify the cosine against your hosted provider before relying on them sharing an index.' },
  { name: 'qwen3-embedding:4b-fp16', label: 'Qwen3 Embedding 4B — fp16 (2560 dims, matches hosted providers)', size: 8000 * 1024 * 1024, dims: 2560, about: 'Full 16-bit weights — the same arithmetic hosted providers run, so its vectors can share an index with OpenRouter/DeepInfra. Costs ~8GB VRAM and is SLOWER than Q4_K_M (embedding is prefill-bound, so 4x the weight bytes means fewer texts/sec). Same 2560 dims, but switching precision still invalidates every existing vector — plan a full re-index.' },
