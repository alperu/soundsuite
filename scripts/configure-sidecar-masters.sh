#!/usr/bin/env bash
# configure-sidecar-masters.sh
#
# Push the correct set of master URLs to every sidecar in the fleet based on
...
#
# Usage:
#   ./scripts/configure-sidecar-masters.sh [--dry-run] [--fleet-url http://localhost:3000]
...
ws_port_for() {
  ...
  case "$port" in
    ...
    3848) echo 3003 ;;
    *)
      # Unknown URL port — default to URL-port + 2 (matches the 3000→3002 / 3848→3850 convention badly but at least documents the gap).
      # Safer: fail loudly so the operator notices.
...
while IFS=$'\t' read -r HOSTNAME SIDECAR_URL; do
  ...
  # Fetch the sidecar's current master list once per host.
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  echo "Done. Verify per-sidecar with:"
