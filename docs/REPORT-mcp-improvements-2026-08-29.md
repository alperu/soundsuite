...
- MCP stdio server named `sound-suite`; translates `GET /api/mcp/tools` → `tools/list` and `tools/call` → `POST /api/mcp/execute` (`{"tool", "arguments"}` shape).
