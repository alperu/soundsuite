...
| [30](./30-mcp-parity-and-fleet-visibility.md) | Parameter parity + read-only fleet tools | S | **P1** | `searchMode`/`recordStatus` exist only on `query_case_knowledge`; `multiPass` is a *synthesis* switch. **Part 3 amended 2026-09-09:** the fleet tool must probe not relay (vLLM has no `/api/ps`), the role→port map exists three times and disagrees, and there is no declared status type at all. Blocked by 39. |
...
| [39](./39-role-aware-readiness.md) | Role-aware readiness — populate the mechanism that exists, retire the global | S–M | P1 | Readiness is ONE cached boolean from a completion-only probe, applied to all six roles: `notReady` read `[]` while the retrieval path hung. A per-tool dependency mechanism already exists and ~9 tools use it — the defect is the `ollamaUp` global layered on top. Declare roles, delete the second gate. Unblocks 30 and 38. |
| [40](./40-fleet-admission-control.md) | Fleet routing spreads load but never refuses it | M | P1 | Load spreading is real and GPU-aware. But `activeRequests` occurs at three lines — the type, the selection read, and a log field — and is **never compared to a limit**: no admission control, no queue, so a saturated fleet makes the fifth caller wait rather than told. Release is fire-and-forget, so the counter can leak and bias routing away from a free host. Concurrency multiplies task 38's budget gap. |
...
| [50](./50-rlm-sandbox-unassignable.md) | `ss-rlm-sandbox` cannot be assigned on any host | S–M | P1 | **Implemented 2026-09-15.** The master always pushes a runtime, so `resolveMode` short-circuits to `resolveModeForRuntime`, which has no `ss-rlm-sandbox` case in any of its four branches → the mode is silently skipped on Mac, Windows **and** Linux. Adds a fifth `docker-cpu` (no-GPU) runtime, wires port 8101 into the roleassign UI, normalizes stale rows, and fixes `RUNTIME_VALUES` dropping stored `docker-model-runner` rows. |

> **Table is stale from 41 onward.** Tasks 41–49 exist as files but were never
> added here; 50 is listed because it was written against this convention. Treat
> `ls docs/tasks/` as the authoritative index until the table is reconciled.
