...
| [35](./35-bulk-promotion.md) | Bulk promotion of the 768 un-ingested documents | M | P0 | The only task that changes what the system can answer. `filingId` is not required to parse, so this is a status update, not 768 filing decisions. Waves by case, smallest first: 30 → 39 → 45 → 252 → 402. Needs 21 **and** 34. |
