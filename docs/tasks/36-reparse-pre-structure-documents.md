...
| # | Item | Status |
|---|---|---|
| 1 | **Identify the pre-structure cohort by absent `parserVersion`** and confirm the count before acting. It will have changed if [task 35](./35-bulk-promotion.md) ran — newly-ingested documents should carry a version, so a *rising* null count after task 35 is itself a defect worth stopping for. ⚠️ The title's "74" is a dated observation, not a constant: re-measure. | ☐ **operator** |
...
| 4 | **Decide explicitly whether the `hybrid-docparse-1` cohort needs re-parsing too.** | ☑ **No.** `src/lib/ingestion/ingestion-pipeline.ts:1039` stamps the string literal `'hybrid-docparse-1'`, and it is the **only** site in `src/` that writes `parserVersion` at all. The current parser writes the same version those documents already carry, so they are not a second cohort — they are the current generation, and re-parsing them costs full price for identical output. Revisit only if that literal is bumped. |
